...
Functional role IDs own baseline/history semantics. Legacy AgentName values remain
on result objects as event-time actor provenance and compatibility identifiers.
"""
...
from ..agents.profiles import ROLE_BASELINES
from ..models import (
    AgentSummary,
    DashboardSnapshot,
    DriftAlert,
...
from ..utils.helpers import format_score, pct
...
class MetricsAggregator:
    """Convert EvalResult objects into summaries and role-based drift alerts."""

    history_key_type: Type[AgentRole] = AgentRole

    def __init__(
        self, baseline: Optional[Dict[AgentRole, Dict[str, Optional[float]]]] = None
    ) -> None:
        self.baseline = baseline or ROLE_BASELINES
        self._history: Dict[AgentRole, List[AgentSummary]] = {}

    @property
    def history(self) -> Dict[AgentRole, List[AgentSummary]]:
        """Role-keyed evaluation history exposed read-only by convention."""
    ...
    def summarise(self, result: EvalResult) -> AgentSummary:
        ...
        avg_latency = (
            sum(r.latency_ms for r in tr) / total_tasks if total_tasks else 0.0
        )
        avg_cost = (
            sum(r.token_cost_usd for r in tr) / total_tasks if total_tasks else 0.0
        )

        # Layer scores → agent summary fields
        faithfulness = result.outcome.faithfulness if result.outcome else 0.0
        tool_accuracy = (
            result.action.tool_selection_accuracy if result.action else 0.0
        )
        ethics_rights = (
        ...
        # Store by functional role. Actor identity remains on the summary itself.
        self._history.setdefault(role, []).append(summary)

        return summary

    def detect_drift(self, current: AgentSummary) -> List[DriftAlert]:
        """Compare a current summary against the functional-role baseline."""
        alerts: List[DriftAlert] = []
        agent_baseline = self.baseline.get(current.role, {})

        comparisons: List[Tuple[str, Optional[float], float]] = [
            (
                "task_success_rate",
                agent_baseline.get("task_success"),
            ...
            (
                "ethics_rights_pass",
                agent_baseline.get("ethics_rights"),
                current.ethics_rights_pass,
            ),
            (
                "archival_quality",
                agent_baseline.get("archival"),
                current.archival_quality or 0.0,
            ),
        ]
            ...
            if delta < -threshold:
                ...
                alerts.append(
                    DriftAlert(
                        ...
                        severity=severity,
                        message=(
                            f"{current.role.value} | {metric_name} dropped "
    ...
    def build_snapshot(
        ...
        return DashboardSnapshot(
            ...
            total_evals_run=total_evals_run
            or sum(s.total_tasks for s in summaries),
        )

    def format_table(self, summaries: List[AgentSummary]) -> str:
        """Return a formatted ASCII table of agent summaries."""
        header = (
            f"{'Role':<40} | {'Task Success':>12} | {'Faithfulness':>12} | "
        ...
        for s in summaries:
            archival = (
                format_score(s.archival_quality)
                if s.archival_quality is not None
                else "  N/A"
            )
            rows.append(
                f"{s.role.value:<40} | {format_score(s.task_success_rate):>12} | "
